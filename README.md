...
holding disk to the landing. Rates read as two truths: the leading figure is the
throughput right now (a trailing ~30s window), while `avg` is measured over the lane's
*busy* time — what it actually spent writing — with `busy N%` its share of the run; a
flush lane waiting for dumps says `idle` outright instead of diluting the average. The report covers the whole cycle: a run
