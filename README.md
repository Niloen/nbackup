...
**slot** is just the set of archives one run produces, grouped by run id — there
is no slot-level index to corrupt or keep in sync. The same shape maps to disk,
an object store, or tape; each medium frames the header its own way.
...
slots/slot-2026-06-21/
  000000-app01_home-L0.tar.zst   # clean compressed tar (the payload)
  000000-app01_home-L0.hdr       # JSON header sidecar (identity)
  000001-db01_pg-L1.tar.zst
  000001-db01_pg-L1.hdr
  …                              # + a small commit footer & file-list index per archive
```

Each archive is made durable by its own **commit footer**, written last (after
the payload and a gzipped **member index** of the files inside) — its presence is
what marks the archive complete, so a crashed run keeps every finished archive
and a rerun just fills in the rest. The footer carries the archive's checksum and
sizes; the member index is what lets `nb recover` browse a backup's file list
...
```bash
zstd -dc 000000-app01_home-L0.tar.zst | tar -xf -
```
...
| `nb dle`             | Show one source's archive timeline across slots         |
