...
func TestClientSidePipelineRoundTrip(t *testing.T) {
	...
	bs, err := m.BackupSource(archiver.BackupRequest{DLE: "app-data", Scope: archiver.Scope{Source: src}, Level: 0, BaseLevel: -1})
	if err != nil {
		t.Fatal(err)
	}
	...
	producer := xfer.NewProgramSource(ex).Add(bs.Stage).Add(comp).Add(enc).
	...
	dest := t.TempDir()
	sink := xfer.NewProgramSink(ex).Add(dec).Add(decomp).Add(m.RestoreStage(dest, nil))
