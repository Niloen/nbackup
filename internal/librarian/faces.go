...
// files, nothing that can change one. It is the clerk's Mounter role plus lifecycle;
...
type WriteMedium interface {
	...
	Volume() media.Volume
	Parallel() bool
	PrepareWrite(appendable bool, expect string, now time.Time, logf Logf) (volName string, epoch int, err error)
	WriteSink(volume string, epoch int, appendable bool, partSize int64, now time.Time, logf Logf) *WriteSink
	LazyDriveSinks(appendable bool, expect string, partSize int64, now time.Time, logf Logf) []*WriteSink
	io.Closer
}
...
type AdminMedium interface {
	Label(name string, relabel, force bool, minAge time.Duration, now time.Time, logf Logf) error
	Load(target string, byLabel bool, logf Logf) error
	View() (View, error)
	Labeled() bool
	AppendOnly() bool
	Volume() media.Volume
