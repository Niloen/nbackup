...
func TestAssertPartWrongVolume(t *testing.T) {
	...
	_, err := NewReader().Open([]record.FilePos{{Pos: 0}}, want, so.open)
	if err == nil {
...
func TestReadMidStreamOpenFailure(t *testing.T) {
	...
	rc, err := NewReader().Open([]record.FilePos{{Pos: 0}, {Pos: 1}}, want, so.open)
	if err != nil {
...
func TestReadPartCloseErrorPropagates(t *testing.T) {
	...
	rc, err := NewReader().Open([]record.FilePos{{Pos: 0}}, want, so.open)
	if err != nil {
		t.Fatalf("prime should succeed: %v", err)
	}
	_, err = io.ReadAll(rc)
...
func TestOpenNoParts(t *testing.T) {
	_, err := NewReader().Open(nil, Ref{Run: "r", DLE: "d", Level: 0}, nil)
...
func TestVerifyPartsWrongVolume(t *testing.T) {
	...
	ok, err := NewReader().VerifyParts([]record.FilePos{{Pos: 0}}, want, "deadbeef", so.open)
	if ok {
