...
// "capacity" option, so this test can exercise MediumResidual's capacity arithmetic
...
// TestMediumResidualExcludesReclaimableBytes checks that MediumResidual reports
...
func TestMediumResidualExcludesReclaimableBytes(t *testing.T) {
	...
	residual, capacity, err := acct.MediumResidual("disk", now)
	if err != nil {
		t.Fatal(err)
	}
	if capacity != 90 {
	...
	// MediumResidualOverCapacity shares this computation: unlike raw used (100 >
	// 90), the residual (60) is well under capacity, so it must not flag over.
	over, overResidual, overCapacity, err := acct.MediumResidualOverCapacity("disk", now)
	if err != nil {
		t.Fatal(err)
	}
	if over {
		t.Errorf("MediumResidualOverCapacity: over = true, want false (residual %d <= capacity %d)", overResidual, overCapacity)
	}
	if overResidual != residual || overCapacity != capacity {
		t.Errorf("MediumResidualOverCapacity = (%d, %d), want the same figures MediumResidual computed (%d, %d)",
			overResidual, overCapacity, residual, capacity)
	}
}

// TestMediumResidualUnknownMedium confirms an unconfigured medium name errors
// rather than silently reporting zero, the way ProfileFor already does.
func TestMediumResidualUnknownMedium(t *testing.T) {
	cat, err := catalog.Open(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	acct := New(Deps{Cat: cat, Cfg: &config.Config{}})
	if _, _, err := acct.MediumResidual("nope", time.Now()); err == nil {
		t.Error("MediumResidual(unknown medium) = nil error, want one")
	}
}
