...
- **Inspect with a noun** — `nb slot`, `nb dle`, `nb medium`. With no argument it
  lists; with an id it details one item (`nb slot slot-2026-06-21.001`,
  `nb medium lto`).
...
landing: disk                        # which medium new slots are created on
...
the dump program (GNU tar), and a **medium** is where slots land.
...
nb slot                # list slots — each is one run
```
...
```bash
nb slot                        # list slots, with a COPIES column
nb slot slot-2026-06-21.001    # archives in one slot + every copy's location
nb dle                         # list sources and their archive timelines
nb dle app01:/home             # one source's history across slots
nb verify --all                # re-check every slot's archive checksums
```

A **slot** is just a directory you can open with `ls`. To prove a restore really
