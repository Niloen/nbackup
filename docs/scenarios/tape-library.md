...
parent: Scenarios
nav_order: 6
description: "A multi-bay changer with automatic labeling, label rotation, and cross-tape spanning."
---

# Robotic tape library
{: .no_toc }

A multi-bay changer where a robot mounts whichever bay is loaded, with automatic labeling, whole-volume label rotation, and cross-tape spanning.
{: .fs-6 .fw-300 }
...
Use this when your tape is a **robotic library**: many physical bays behind one
drive, where a robot mounts whichever bay is loaded. NBackup addresses the bays,
reads each tape's label after mounting, and rolls from one tape to the next on its
own — so runs pack onto tapes, fill them, span across them, and recycle aged-out
volumes without an operator standing by.
...
```bash
nb label lto lto-0001        # label a blank bay (or rely on auto_label)
nb medium lto                # inventory the library: each bay → its label + what's loaded
...
- **Labels are verified before every write.** Each tape carries a self-describing
  identity label. NBackup reads it after mounting a bay and checks it before writing, so
  a foreign or wrong-pool reel is **never clobbered**.
...
- **A run that fills a tape mid-write spans onto the next automatically** — splitting even
  a single large archive. The robot mounts the next writable bay: a blank (auto-labeled),
...
- **Tape reclaims whole volumes, never per-archive.** Capacity is `bays × volume_size`;
  `nb prune` rotates labels rather than deleting individual archives from a tape. See
  [Pruning](../features/pruning).

## Restore

Restore mounts whichever tape holds the copy it needs. A **spanned** archive reassembles
by mounting its tapes in order.

## Single-drive variants

A single drive you change by hand — the disk-emulated station (`mode: manual`) or a real
