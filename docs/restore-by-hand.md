...
each archive is a clean `NNNNNN-<dle>-L<n>.tar.<codec>` payload with a `.hdr`
sidecar, plus a small commit footer and member index per archive.

## A single full

The disk payload is a clean `tar.<codec>` — no header to skip:

```bash
zstd -dc 000000-app01_home-L0.tar.zst | tar -xf -
```

(`gzip -dc` for a `.tar.gz`; a `none` codec is a plain `.tar`.)
...
An encrypted archive keeps the same `.tar.<codec>` name and reverses the same way,
decrypting first:

```bash
gpg -d < 000000-app01_home-L0.tar.zst | zstd -dc | tar -xf -
```
