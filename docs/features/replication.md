...
parent: Features
nav_order: 4
description: "Land fast on disk, replicate offsite. nb copy moves one slot between media; nb sync mirrors a whole medium."
---
...
disk (cheap, fast, online), then mirror the committed slots to tape or S3 for the
offsite copy. NBackup has two commands for moving slots between media:

- `nb copy` copies **one** slot between media, choosing the endpoints with
  `--from`/`--to` (e.g. disk → tape).
- `nb sync` is the **batch** form of `nb copy`: it copies every slot the target
  medium is missing.
...
writing. It is **idempotent**: each slot copies atomically and records a second
...
slots offsite and `nb prune` independently trims disk back to its budget. A
