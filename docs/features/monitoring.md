...
For the classic **per-DLE dump report**, `nb report --dump` prints the latest dump
in detail — each DLE's level, original/output size, compression %, files, dump
time, and rate, with full/incremental totals:
